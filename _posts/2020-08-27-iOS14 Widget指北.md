...
Link可用于备忘录和日历小部件，点击不同的备忘录和日期直接跳转到对应的备忘录详情和代办详情页面
...
创建完小部件之后，我们会多出一个SmileEverydayWidget.swift文件，这已经是一个可以run起来的小部件了，因为我们接下来要逐个方法来分析，所以将文件全文展示如下
...
为了在小部件库中显示小部件，WidgetKit 要求提供者提供预览快照， 即`snapshot()`，这个方法里主要提供了一些`示例数据，最好是真实数据`，用于`当时间线不能离开展示`的时候展示给用户，例如没有还没有从服务端获取到所需数据是，不至于展示白屏给用户看。

需要说明的是，快照是系统需要快速显示单个条目的位置。

因此，您的扩展程序有望尽快返回视图，因为这样做时，您会在iOS上漂亮的Widget Gallery中看到真正的Widget。

这不是我们在设计时必须提供的屏幕截图或图像。这是您在iOS，iPadOS和macOS上真正的小部件体验。
